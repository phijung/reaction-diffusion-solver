...
py::array_t<double> laplacian_serial(py::array_t<double, py::array::c_style | py::array::forcecast> input) {
...
py::array_t<double> laplacian_parallel(py::array_t<double, py::array::c_style | py::array::forcecast> input) {
    auto conc = input.unchecked<2>();
...
PYBIND11_MODULE(laplacian_pybind, m) {
    ...
    m.def("laplacian_serial", &laplacian_serial, "Compute the laplacian");
    m.def("laplacian_parallel", &laplacian_parallel, "Compute the laplacian with openMP");
}
